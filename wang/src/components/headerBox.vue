<template>
    <div class="container">
        ...
        <div class="headerBox clearFix">
            <img class="logo" @click="toIndex" src="../assets/img/header/logo.png" alt="">
            <div class="headerRight fr">
                <!--已登录-->
                <div v-if="registerState" class="displayInlineBlock already">
                    <el-dropdown class="middleBox" @command="handleCommand" trigger="click">
                ...
                <div v-else class="future">
                    <el-button @click="loginBtn" class="whiteColor font16 loginBtn" type="text">
                        {{$t('header.login')}}
                    </el-button>
                    <el-button @click="joinNowBtn" type="default" class=" joinNow font16">
                        {{$t('header.join')}}
                    </el-button>
                </div>
                <div class="future">
            ...
            <div class="background">
                ...
                <el-form :model="loginForm" auto-complete="off">
                    ...
                    <el-form-item class="marginBottom0">
                        <el-button class="login" @click="handelLogin">{{$t('loginRes.loginBtn')}}</el-button>
                    </el-form-item>
                ...
                <div class="register">
                    <el-button @click="register" class="btn" type="text">{{$t('loginRes.loginResText')}}</el-button>
                </div>
            </div>
        </el-dialog>
        <!--弹框- 注册-->
        <el-dialog
            ...
            class="registerPop"
            append-to-body
            :before-close="resetJoinNow"
            :close-on-click-modal="false"
            :close-on-press-escape="false">
            <div class="background">
                <!--注册标题-->
                <span class="loginText">{{$t('loginRes.registerTitle')}}</span>
                <div class="mailbox">
                    <p class="email">{{$t('loginRes.loginTxt')}}</p>
                    <div class="userName">
                        <img src="../assets/img/login/user.png" alt="">
                        <el-input
                            class="formInput"
                            v-model="Register.userName"
                            @blur="blurInput(Register,'userNameFlag',Register.userName,'userNameTip')"
                            :placeholder="$t('loginRes.userNamePlaceholder')"/>
                    </div>
                    <div class="registerTip">
                        <span v-if="Register.userNameFlag" class="isNotNullText">{{Register.userNameTip}}</span>
                    </div>
                </div>
                <div class="mailbox">
                    ...
                    <div class="password">
                        ...
                        <el-input
                            class="formInput"
                            type="password"
                            @blur="blurInputRegister(Register,'passwordFlag','passwordTip',Register.password,Register.ensPassword)"
                            v-model="Register.password"
                            :placeholder="$t('loginRes.passwordText')"/>
                    </div>
                    <div class="registerTip">
                        <span v-if="Register.passwordFlag" class="isNotNullText">{{Register.passwordTip}}</span>
                    </div>
                </div>
                <div class="mailbox">
                    ...
                    <div class="password">
                        ...
                        <el-input
                            class="formInput"
                            type="password"
                            @blur="blurInputRegister(Register,'ensPasswordFlag','ensPasswordTip',Register.password,Register.ensPassword)"
                            v-model="Register.ensPassword"
                            :placeholder="$t('loginRes.passwordText')"/>
                    </div>
                    <div class="registerTip">
                        <span v-if="Register.ensPasswordFlag" class="isNotNullText">{{Register.ensPasswordTip}}</span>
                    </div>
                </div>
                <div>
                    <el-checkbox v-model="checked" class="fl marginRight10"></el-checkbox>
                    <div class="fl">{{$t('header.agree')}}</div>
                    <div @click="userAgreement" class="user">《{{$t('header.userXieyi')}}》</div>
                </div>
                <el-button @click="registerSuccessful" class="registerBtn">{{$t('loginRes.registerBtn')}}</el-button>
                <div class="register">
                    <el-button @click="toLogin('account')" class="btn" type="text">{{$t('loginRes.hasUserName')}}
                    </el-button>
                </div>
            </div>
        </el-dialog>
        ...
        <el-dialog
            ...
            :close-on-press-escape="false"
            :close-on-click-modal="false">
            <el-form :model="passwordForm" label-width="140px" autocomplete="off">
                <el-form-item class="passwordInput" :label="$t('loginRes.oldPassword') + '：'">
                    <el-input
                        type="password"
                        name="password"
                        auto-complete="new-password"
                    ...
                    <div class="passwordForm">
                        <span
                            v-if="passwordForm.oldPasswordFlag"
                            class="isNotNullText">{{passwordForm.oldPasswordTip}}</span>
                    </div>
                </el-form-item>
                <el-form-item class="passwordInput" :label="$t('loginRes.password') + '：'">
                    <el-input
                        type="password"
                        name="password"
                        auto-complete="new-password"
                    ...
                    <div class="passwordForm">
                        <span
                            v-if="passwordForm.passwordFlag"
                            class="isNotNullText">{{passwordForm.passwordTip}}</span>
                    </div>
                </el-form-item>
                <el-form-item class="passwordInput" :label="$t('loginRes.confirmPassword') + '：'">
                    <el-input
                        type="password"
                        name="password"
                        auto-complete="new-password"
                    ...
                    <div class="passwordForm">
                        <span
                            v-if="passwordForm.confirmPasswordFlag"
                            class="isNotNullText">{{passwordForm.confirmPasswordTip}}</span>
                    </div>
                </el-form-item>
            </el-form>
            <div slot="footer">
                <el-button type="default" @click="resetPwdBox">{{$t('public.cancel')}}</el-button>
                <el-button type="default" @click="addPassword">{{$t('public.confirm')}}</el-button>
            </div>
        </el-dialog>
        <!-- 协议弹出框 -->
        <el-dialog
            ...
            <div class="background">
                ...
                <el-form :model="photoForm">
                    ...
                    <el-form-item class="marginBottom0 tc register">
                        <el-button @click="toLogin('phone')" type="text" class="btn">{{$t('loginRes.hasUserName')}}
                        </el-button>
                    </el-form-item>
                </el-form>
            </div>
        ...
        <el-dialog
            ...
            <div class="background">
                ...
                <el-form :model="emailForm">
                    ...
                    <el-form-item class="marginBottom0 tc register">
                        <el-button @click="toLogin('email')" type="text" class="btn">{{$t('loginRes.hasUserName')}}
                        </el-button>
                    </el-form-item>
                </el-form>
            </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                Register: {
                    ...
                    passwordFlag: false, // 密码标识
                    passwordTip: "", // 密码提示
                    ensPasswordFlag: false, // 确认密码标识
                    ensPasswordTip: "" // 确认密码提示
                },
                // 语言表单
                formItem: {
        ...
        methods: {
            ...
            resetRegTip() {
                ...
                // 没有登录成功
                if (_t.$route.name === "USER_INFO") {
                    _t.$bus.emit("getUserInfo", true);
                } else {
                    _t.$router.push({name: "USER_INFO"});
                }
            },
            // 忘记密码按钮
            forgotBtn() {
            ...
            saveForgotPwdByPhone() {
                ...
                // 可以提交
                if (_t.forgotItem.photoNumFlag === false
            ...
            resetEmail() {
                let _t = this;
                _t.dialogEmail = false; // 邮箱蒙版
                _t.emailForm.emailNum = ''; // 邮箱地址
                _t.emailForm.emailNumFlag = false; // 邮箱地址标识
                _t.emailForm.emailNumTip = ''; // 邮箱地址提示
            ...
            imgClick(item) {
                let _t = this;
                // 部分三方登录注释
                if (item.type === 'facebook' || item.type === 'twitter' || item.type === 'ins') {
            ...
            addPassword() {
                ...
                // 用户id
                var userId = _t.getCookie("WJE_USER_ID");
                    ...
                    if (_t.passwordForm.passwordFlag === false
                        ...
                        _t.$api.post("user/updateUserPwd", {
                            ...
                            switch (res.status) {
                                case 200:
                                    _t.resetPwdBox();
                                    _t.$confirm(_t.$t("public.changeSuccess"), _t.$t("public.warmPrompt"), {
            ...
            handleCommand(item) {
                ...
                if (item === "pwd") {
                    ...
                    // 个人资料
                    _t.$jump.toJumpPage('PERSONAGE_DATA', 'M_PERSONAGE_DATA');
                }
            },
            // 点击logo回首页
            toIndex() {
                ...
                // 新闻详情页面不跳转首页
                if (routerName !== 'HOME' && routerName !== 'NEWS_DETAILS' && routerName !== 'M_NEWS_DETAILS') {
                    _t.$jump.toJumpPage('HOME', 'M_HOME');
                } else if (routerName === 'HOME') {
                    _t.$bus.emit('getAnimation_pc', true);
                }
            },
            // 点击LOGIN按钮显示弹出层
            loginBtn() {
            ...
            resetJoinNow() {
                let _t = this;
                _t.dialogRegister = false; // 账号注册蒙版
                _t.Register.userName = ''; //用户名
                _t.Register.password = ''; // 密码
                _t.Register.ensPassword = ''; // 确认密码
                _t.Register.userNameFlag = false; // 用户名标识
                _t.Register.userNameTip = ''; // 用户名提示
                _t.Register.passwordFlag = false; // 密码标识
                _t.Register.passwordTip = ''; // 密码提示
                _t.Register.ensPasswordFlag = false; // 确认密码标识
                _t.Register.ensPasswordTip = ''; // 确认密码提示
            },
            // 失去焦点校验
            ...
            // 失去焦点校验密码和确认密码
            blurInputRegister(item, flag, tip, pwd, ensPwd) {
                var _t = this;
                if (flag === "passwordFlag") {
                    // 校验密码
                    if (pwd.trim() === "") {
                    ...
                    if (ensPwd.trim() === "") {
                        ...
                        if (pwd.trim() !== ensPwd.trim()) {
                            item[flag] = true;
                            item[tip] = _t.$t("public.password");
                        } else {
                            // 密码不为空
                            item[flag] = false;
                            item[tip] = "";
                        ...
                        switch (res.status) {
                            case 200: // 成功
                                ...
                                if (res.data.status === 2) {
                                    ...
                                    if (_t.$route.name === "VIP_PRIVATE") {
                                        _t.$bus.emit("loginVip", true);
                                    }
                                    // 登录之后判断 是否 lifeStyle 登录
                                    if (_t.$route.name === 'LIFESTYLE') {
                                        _t.$bus.emit('getDynamicUser_pc', true);
                                    }
                                }
                                break;
                            case 1000: // 账户锁定
            ...
            registerSuccessful() {
                var _t = this;
                // 校验账户名
                _t.blurInput(_t.Register, "userNameFlag", _t.Register.userName, "userNameTip");
                // 校验密码和确认密码
                _t.blurInputRegister(_t.Register, "passwordFlag", "passwordTip", _t.Register.password, _t.Register.ensPassword);
                // 校验确认密码
                _t.blurInputRegister(_t.Register, "ensPasswordFlag", "ensPasswordTip", _t.Register.password, _t.Register.ensPassword);
                // 校验协议
                if (_t.checked === false) {
                    _t.$message.error(_t.$t('loginRes.selectAgreement'));
                    return;
                ...
                // 可以提交
                if (_t.Register.userNameFlag === false &&
                    _t.Register.passwordFlag === false &&
                    _t.Register.ensPasswordFlag === false &&
                    _t.checked === true) {
                    _t.$api.post("user/register", {
                        username: _t.Register.userName.trim(),
                        userPwd: _t.$md5('#$%TEAMWANG%$#' + _t.Register.password.trim() + '#$%JACKSONWANG%$#'),
                        reUserPwd: _t.$md5('#$%TEAMWANG%$#' + _t.Register.ensPassword.trim() + '#$%JACKSONWANG%$#'),
                        registerWay: "0" // 注册方式 0: 账号注册
                    }, function (res) {
                        _t.registerState = false;
                        switch (res.status) {
                            case 200: // 注册成功
                                ...
                                // 清空输入框的值
                                _t.Register.userName = "";
                                _t.Register.password = "";
                                _t.Register.ensPassword = "";
                                // 缓存token
                                ...
                                break;
                            case 1004: // 已注册
                                _t.dialogRegister = true;
                                _t.Register.userNameTip = res.message;
                                // 显示用户注册提示
                                _t.Register.userNameFlag = true;
                                break;
                            default:
                                // 注册失败
                                _t.$message.error(res.message);
            ...
            initThird() {
                ...
                // 第一层判断 是否登录
                var isLogin = _t.getCookie("WJE_IS_LOGIN");
                if (isLogin !== null
                    && isLogin !== "null"
                    ...
                    // 未登录 拦截然后去登录
                    if (window.location.pathname.indexOf("qq") !== -1 && _t.$route.name !== "USER_INFO") {
                        // 拦截qq
                        _t.loginThird("qq", code, state);
                    } else if (window.location.pathname.indexOf("wechat") !== -1 && _t.$route.name !== "USER_INFO") {
                        // 拦截微信
                        _t.loginThird("wechat", code, state);
                    } else if (window.location.pathname.indexOf("weibo") !== -1 && _t.$route.name !== "USER_INFO") {
                        // 拦截微博
                        _t.loginThird("weibo", code, state);
                    } else if (window.location.pathname.indexOf("facebook") !== -1 && _t.$route.name !== "USER_INFO") {
                        // 拦截facebook
                        _t.loginThird("facebook", code, state);
                    } else if (window.location.pathname.indexOf("twitter") !== -1 && _t.$route.name !== "USER_INFO") {
                        // 拦截 twitter
                        _t.loginThird("twitter", code, state);
                    } else if (window.location.pathname.indexOf("insgram") !== -1 && _t.$route.name !== "USER_INFO") {
                        // 拦截 insgram
...
<style scoped>
    ...
    .LoginPop .background {
        ...
        padding: 50px 64px 37px 64px;
    }

    .LoginPop .background .loginText {
    ...
    .LoginPop .background .userName img {
        display: inline-block;
        margin-right: 23px;
        width: 23px;
    }

    .LoginPop .background .password {
        display: flex;
        align-items: center;
        border-bottom: 1px solid #57564c;
        margin-bottom: 0;
    ...
    .LoginPop .background .password img {
        display: inline-block;
        margin-right: 23px;
        width: 23px;
    }

    .LoginPop .background .radio {
        display: flex;
        justify-content: space-between;
        align-items: center;
    }

    .LoginPop .background .radio .btn,
    .LoginPop .background .radio .redBox {
        color: #d8d9d6;
    }

    .LoginPop .background .login {
        margin-bottom: 0;
        width: 100%;
        background: none;
        color: #888986;
    ...
    .LoginPop .background .brandIcon img {
        width: 24px;
        height: 22px;
        cursor: pointer;
    }

    .registerPop .background .register,
    .LoginPop .background .register {
        width: 100%;
        text-align: center;
    }

    .registerPop .background .register .btn,
    .LoginPop .background .register .btn {
        color: #888986;
    }

    /*注册*/
    .registerPop .background {
        background: rgba(0, 0, 0, 0.7);
        padding: 50px 64px 37px 64px;
    }

    .registerPop .background .mailbox {
        margin-bottom: 10px;
    }

    .registerPop .background .registerTip {
        height: 20px;
        line-height: 20px;
        padding-left: 40px;
    }

    .registerPop .background .loginText {
        display: inline-block;
        font-size: 30px;
        height: 30px;
        line-height: 30px;
    ...
    .registerPop .background .email {
        font-size: 16px;
        margin-bottom: 10px;
        line-height: 22px;
    }

    .registerPop .background .registerBtn {
        margin-top: 10px;
        margin-bottom: 30px;
        width: 100%;
        background: none;
    ...
    .registerPop .background .userName {
        display: flex;
        align-items: center;
        border-bottom: 1px solid #57564c;
    }

    .registerPop .background .userName img {
        display: inline-block;
        margin-right: 23px;
    }

    .registerPop .background .passwordText {
        font-size: 18px;
    }

    .registerPop .background .password {
        display: flex;
        align-items: center;
        border-bottom: 1px solid #57564c;
    }

    .registerPop .background .password img {
        display: inline-block;
        margin-right: 23px;
        width: 23px;
        height: 23px;
...
<style>
    .languageSelect .el-input__inner {
        background-color: transparent;
        color: #fff;
        padding: 0 !important;
    ...
    .registerPop .el-dialog__body,
    .LoginPop .el-dialog__body {
        color: #fff;
        padding: 0;
    }

    .registerPop .el-dialog__header,
    .LoginPop .el-dialog__header,
    .LoginPop .background .formInput .el-input__inner {
        padding: 0 !important;
    }

    .registerPop .el-input .el-input__inner {
    ...
    /*修改密码*/
    .resetPassword .el-dialog {
        background-color: rgba(0, 0, 0, 1);
        background-image: url("../assets/img/common/mask_bg.png");
        background-repeat: repeat;
    }

    .resetPassword .el-dialog__header {
    ...
    .LoginPop .background .login:hover,
    .registerPop .background .login:hover,
    .registerPop .background .registerBtn:hover,
    .resetPassword .passwordInput .el-form-item__label,
    .resetPassword .el-dialog__header .el-dialog__title {
        color: #fff;
    }

    .resetPassword .passwordInput .el-input__inner {
        padding: 0 !important;
        color: #fff;
